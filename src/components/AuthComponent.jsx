    ...
    try {
      if (isLogin) {
        ...
        const { data, error: signUpError } = await supabase.auth.signUp({
          ...
          password,
          options: {
            // CRITICAL: Must pass the meta-data here so the LiveMatches Component knows what role to render!
        ...
        if (data?.user && data.user.identities && data.user.identities.length === 0) {
          ...
          setMsg("Check your email for the confirmation link! (If you disabled email confirmations, you can just click Sign In!).");
        }
      }
    } catch (err) {
